...
class Pipeline:
    ...
    def process(self, glyphPath):
            ...
            match parsed["type"]:
                case "glyph":
                    ...
                    create_grayscale_image(
                        bitmap, Pipeline.folder / self.name / f"{Pipeline.count}.png"
                    )
                case "geometry":
                    parsed = parsed["data"]
                    ...
                    type_geo = parsed["type"]
                    texts.setdefault(key, []).append((type_geo, toAdd))
        ...
        texts = {
            ...
            if (len(v) > 1 or len(v[0]) > 1) and name in k
        }
        ...
        texts_g = {
            z: list(g)
            for z, g in groupby(texts, key=lambda line: zoom_from_key(line[0]))
        }
